...
def validar_token(token: str = Depends(security)):
    ...
    if not validatoken:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Token invalido ou expirado, faça o login novamente.")
...
@router_bakeshop.post("/login", status_code=status.HTTP_200_OK, tags=["Login"], description="Endpoint para login e geração de token",
...
@router_bakeshop.post("/jogadas/gerar", status_code=status.HTTP_200_OK, tags=["Admin"], description="Gerar jogadas para o cliente",
...
@router_bakeshop.get("/voucher/{loja}/{data}", status_code=status.HTTP_200_OK, tags=["Admin"], description="Consultar o vouchers ativados na data",
...
def consultar_voucher(loja: str, data: str, token: str = Depends(validar_token)):
    ...
    if response == []:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Não existe vouchers utilizados no periodo informado.")
